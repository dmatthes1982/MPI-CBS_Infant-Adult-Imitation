function INFADI_easyMultiPowPlot(cfg, data)
% INFADI_EASYMULTIPOWPLOT is a function, which makes it easier to plot the
% power of all electrodes within a specific condition on a head model.
%
% Use as
%   INFADI_easyMultiPowPlot(cfg, data)
%
...
cfg.condition = ft_getopt(cfg, 'condition', 4);
...
cfg.condition= INFADI_checkCondition( cfg.condition);                       % check cfg.condition definition
...
if ~isempty(cfg.baseline)
  cfg.baseline    = INFADI_checkCondition( cfg.baseline );                  % check cfg.baseline definition
