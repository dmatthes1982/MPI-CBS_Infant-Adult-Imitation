...
fileList     = dir([tmpPath, 'INFADI_d*_08b_pwelch_*.mat']);
...
fileList     = dir([tmpPath, ['INFADI_d*_08b_pwelch_' sessionStr '.mat']]);
fileList     = struct2cell(fileList);
fileList     = fileList(1,:);
numOfFiles  = length(fileList);
...
for i = 1:1:length(fileList)
  ...
  warning off;
  T(3*i-2, 2:end) = num2cell(data_pwelch.experimenter.numOfAllSeg');
  T(3*i-1, 2:end) = num2cell(data_pwelch.experimenter.numOfGoodSeg');
