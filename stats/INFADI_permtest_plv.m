% INFADI_PERMTEST_PLV - Using this script one can test if the difference of
...
cprintf([1,0.4,1], '<strong>Permutation test on plv level</strong>\n');
...
fileList     = dir([srcPath, 'INFADI_d*_07a_plvTheta_*.mat']);
...
for dyad=1:1:numOfFiles
  fileListCopy{dyad} = strsplit(fileList{dyad}, '07a_plvTheta_');
...
fileList     = dir([srcPath 'INFADI_d*_07a_plv' passband '_' sessionStr ...
                    '.mat']);
fileList     = struct2cell(fileList);
...
desPath = [datastorepath 'DualEEG_INFADI_results/PLV_stats/' sessionStr ... % destination path
          '/'];
...
while selection == false
  ...
  if ~all(isstrprop(identifier{1}, 'alphanum'))                             % check if identifier is valid
    ...
    matFile = [desPath 'INFADI_plvStats_' identifier{1} '_' sessionStr ...  % build filename
                '.mat'];
