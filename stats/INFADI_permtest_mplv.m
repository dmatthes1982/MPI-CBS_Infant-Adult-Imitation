% INFADI_PERMTEST_MPLV - Using this script one can test if the difference
% of one connection (e.g. Cz-Cz) or one cluster of connections is
% significant between two conditions. First, a paired-sample t-test is
% conducted. If this test shows a significant result, a permutation test
% follows. Here, the permutation is done on the level of averaged PLV
% values. Since our null hypothesis says that we expect no difference
% between conditions but a difference between dyads, the permutation is
% done only by randomly interchanging the values of the two conditions
% within the dyads. It is highly recommended to only use this script, if
% the number of PLV segments is nearly the same in both conditions for
% single dyads. Otherwise one should use INFADI_PERMTEST_PLV
%
% See also INFADI_PERMTEST_PLV
...
for i = 1:1:numOfFiles
  listOfPart(i) = sscanf(fileList{i}, ['INFADI_d%d_07b_mplv' passband ...   % generate a list of all available numbers of dyads
                                        '_' sessionStr '.mat']);
end
...
filepath = fileparts(mfilename('fullpath'));
load(sprintf('%s/../general/INFADI_generalDefinitions.mat', filepath), ...  % load general definitions
     'generalDefinitions');
