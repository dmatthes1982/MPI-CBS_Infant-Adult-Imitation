function [ data_out ] = INFADI_estNoisyChan( data_in )
% INFADI_ESTNOISYCHAN is a function which is detecting automatically noisy
...
%
% Use as
%   [ data_out ] = INFADI_estNoisyChan( cfg, data_in )
%
% where input data has to be the result of INFADI_CONCATDATA.
%
% Reference:
...
% Check data
% -------------------------------------------------------------------------
if numel(data_in.experimenter.trialinfo) ~= 1 || numel(data_in.child.trialinfo) ~= 1
  error('Dataset has more than one trial. Data has to be concatenated!');
end

for i = 1:1:2
  if i == 1
    fprintf('<strong>Estimating noisy channels of experimenter...</strong>\n');
    data = data_in.experimenter;                                            % extract data of experimenter
  elseif i == 2
  ...
  if i == 1
    data_out.experimenter = data;                                           % reassign result to experimenter
  elseif i == 2
    data_out.child = data;                                                  % reassign result to child
  end
end
