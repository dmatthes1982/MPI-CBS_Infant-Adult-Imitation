...
fileList     = dir([srcPath, 'INFADI_d*_08b_pwelch_*.mat']);                % determine all avaible sessions
...
fileList     = dir([srcPath 'INFADI_d*_08b_pwelch_' sessionStr '.mat']);
fileList     = struct2cell(fileList);
...
for i = 1:1:numOfFiles
  listOfPart(i) = sscanf(fileList{i}, ['INFADI_d%d_08b_pwelch_' ...         % generate a list of all available numbers of dyads
                                        sessionStr '.mat']);
end
...
freqNum   = data_pwelch.experimenter.freq;
...
labelOrig = data_pwelch.experimenter.label;                                 % extract channel names
...
desPath = [path 'DualEEG_INFADI_results/PSD_export/general/' sessionStr ... % destination path
          '/'];
...
for dyad = 1:1:numOfFiles
  ...
  if any(~strcmp(data_pwelch.experimenter.label, labelOrig))
  ...
  for trl=1:1:numOfTrials
    waitbar(((dyad-1)*numOfTrials + trl)/(numOfFiles * numOfTrials), ...
                  f, 'Please wait...');
    loc_trl = ismember(data_pwelch.experimenter.trialinfo, condNum(trl));   % participant 1
    if any(loc_trl)
      psdPart1 = squeeze(data_pwelch.experimenter.powspctrm(loc_trl,:,:));
      loc_freq = ismember(data_pwelch.experimenter.freq, freqNum);
      loc_chan = ismember(data_pwelch.experimenter.label, label);
    ...
    if any(loc_trl)
      psdPart2 = squeeze(data_pwelch.child.powspctrm(loc_trl, :, :));
